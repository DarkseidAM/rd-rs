...
impl<FS: Filesystem + Send + Sync + 'static> Session<FS> {
        ...
        loop {
            ...
            match opcode {
                ...
                fuse_opcode::FUSE_STATX => {
                    self.handle_statx(request, in_header, data_ref, &fs).await;
                }
    ...
    async fn handle_statx(
        ...
        use crate::raw::abi::{fuse_attr, fuse_statx, fuse_statx_in, fuse_statx_out, fuse_sx_time};

        let statx_in = match fuse_statx_in::read_from_prefix(data) {
            Err(err) => {
                error!(
                    "deserialize fuse_statx_in failed {}, request unique {}",
                    err, request.unique
                );
                self.response_sender()
                    .send_err(&request, libc::EINVAL.into())
            ...
            Ok((statx_in, _)) => statx_in,
        };

        let resp_sender = self.response_sender().clone();
        let fs = fs.clone();
        let nodeid = in_header.nodeid;
        let getattr_flags = statx_in.getattr_flags;
        let fh = if getattr_flags & FUSE_GETATTR_FH > 0 {
            Some(statx_in.fh)
        } else {
            None
        };

        spawn(debug_span!("fuse_statx"), async move {
            debug!("statx unique {} inode {}", request.unique, nodeid);

            match fs
                .getattr(request, nodeid, fh, getattr_flags)
                .await
            {
                Err(err) => {
                    resp_sender.send_err(&request, err).await;
                ...
                Ok(attr) => {
                    let a: fuse_attr = attr.attr.into();
                    let stat = fuse_statx {
                        mask: 0x7ff, // STATX_BASIC_STAT
                        blksize: a.blksize,
                        attributes: 0,
                        nlink: a.nlink,
                        uid: a.uid,
                        gid: a.gid,
                        mode: (a.mode & 0xffff) as u16,
                        __spare0: [0],
                        ino: a.ino,
                        size: a.size,
                        blocks: a.blocks,
                        ...
                        ctime: fuse_sx_time {
                            tv_sec: a.ctime as i64,
                            tv_nsec: a.ctimensec,
                            __reserved: 0,
                        },
                        mtime: fuse_sx_time {
                            tv_sec: a.mtime as i64,
                            tv_nsec: a.mtimensec,
                            __reserved: 0,
                        },
                        rdev_major: (a.rdev >> 8) as u32,
                        rdev_minor: (a.rdev & 0xff) as u32,
                        dev_major: 0,
                        dev_minor: 0,
                        __spare2: [0; 14],
                    };
                    ...
                    let out_header = fuse_out_header {
                        len: (FUSE_OUT_HEADER_SIZE + mem::size_of::<fuse_statx_out>()) as u32,
                        error: 0,
                        unique: request.unique,
                    };
                    resp_sender
                        .send2(&out_header, out.as_bytes())
                        .await;
                }
            };
        });
    ...
    async fn handle_readdirplus(
        ...
        spawn(debug_span!("fuse_readdirplus"), async move {
            ...
            while let Some(entry) = entries.next().await {
                ...
                // Include padding in the check so the reply never exceeds max_size (kernel rejects oversized replies with EIO).
